...
\lean{ConwayO7.card_support_of_pow_prime_eq_one}\leanok
\end{lemma}
...
\lean{ConwayO7.card_filter_fixed_add_card_support}\leanok
\end{lemma}
...
\]
\lean{ConwayO7.card_filter_fixed_modEq}\leanok
\end{lemma}
...
$\pi$ is $[p^m]$.
\lean{ConwayO7.cycleType_eq_replicate_of_fixed}\leanok
\end{proposition}
...
\lean{ConwayO7.commonNeighbors_eq_singleton_of_adj}\leanok
\uses{def:srg}
\end{lemma}
...
vertices $a \ne b$.
\lean{ConwayO7.commonNeighbors_eq_pair_of_not_adj}\leanok
\uses{def:srg}
\end{lemma}
...
\lean{ConwayO7.eq_of_commonNeighbors_eq}\leanok
\uses{def:srg}
\end{lemma}
...
\lean{ConwayO7.mapsTo_commonNeighbors}\leanok
\end{lemma}
...
\lean{ConwayO7.eq_one_of_fixes_neighbors}\leanok
\uses{def:srg}
\end{proposition}
...
vertex.
\lean{ConwayO7.card_filter_fixed_eq_one}\leanok
\uses{def:srg}
\end{theorem}
...
\begin{theorem}[{Cycle type $[1,7^{14}]$}]\label{thm:grp-cycle-type}
With $\Gamma$ and $\pi$ as in Theorem~\ref{lem:fixed}, the cycle type of
$\pi$ is $[7^{14}]$: one fixed vertex and fourteen $7$-cycles.
\lean{ConwayO7.cycleType_eq_replicate}\leanok
\uses{def:srg, lem:fixed}
\end{theorem}

\begin{proof}\leanok
\uses{lem:grp-fix-support}
Since $\ord\pi = 7$ is prime, the cycle type is $[7^{n+1}]$ for some $n$.  By
Theorem~\ref{lem:fixed} and Lemma~\ref{lem:grp-fix-support} the support has
$99 - 1 = 98$ elements, and the cycle lengths sum to the support size, so
$7(n+1) = 98$ and the cycle type is $[7^{14}]$.
\end{proof}

\section{The automorphism group}

\begin{definition}[Forgetting an automorphism to a permutation]\label{def:grp-autToPerm}
For a graph $G$ on a vertex set $V$, the map sending an automorphism
$\sigma \in \Aut G = (G \simeq_g G)$ to its underlying vertex bijection is a
monoid homomorphism $\Aut G \to \operatorname{Sym}(V)$.
\lean{ConwayO7.autToPerm}\leanok
\end{definition}

\begin{lemma}[Injectivity]\label{lem:grp-autToPerm-injective}
The homomorphism of Definition~\ref{def:grp-autToPerm} is injective: an
automorphism is determined by its action on vertices.
\lean{ConwayO7.autToPerm_injective}\leanok
\uses{def:grp-autToPerm}
\end{lemma}

\begin{lemma}[Finiteness of the automorphism group]\label{lem:grp-aut-finite}
If $V$ is finite then $\Aut G$ is a finite group.
\lean{ConwayO7.instFiniteIso_conwayO7}\leanok
\uses{def:grp-autToPerm}
\end{lemma}

\begin{proof}\leanok
\uses{lem:grp-autToPerm-injective}
$\Aut G$ injects into the finite group $\operatorname{Sym}(V)$.
\end{proof}

\begin{lemma}[Cauchy step]\label{lem:cauchy}
If $7 \mid |\Aut G|$ for a graph $G$ on a finite vertex set, then $\Aut G$
contains an element of order $7$.
\lean{ConwayO7.exists_aut_orderOf_eq_seven, exists_prime_orderOf_dvd_card}\leanok
\uses{lem:grp-aut-finite}
\end{lemma}

\begin{proof}\leanok
Cauchy's theorem for the finite group $\Aut G$
(mathlib's \texttt{exists\_prime\_orderOf\_dvd\_card}) with the prime $7$.
\end{proof}
...
cycle type $[7^{14}]$.
\lean{ConwayO7.cycleType_autToPerm}\leanok
...
ConwayO7.sigma0_val}\leanok
\uses{lem:grp-sigma0-inverse}
\end{definition}

\begin{lemma}[Inverse laws]\label{lem:grp-sigma0-inverse}
The forward and backward block rotations are mutually inverse: advancing the
position within a block by $1$ and then by $6$ (or vice versa) is the
identity, and both maps fix $0$.  Hence $\sigma_0$ is a well-defined
permutation with an explicit inverse.
\lean{ConwayO7.sigma0_leftInverse, ConwayO7.sigma0_rightInverse}\leanok
\end{lemma}
...
\lean{ConwayO7.sigma0_iterate_val}\leanok
\uses{def:sigma0}
\end{lemma}
...
$\sigma_0^7 = 1$.
\lean{ConwayO7.sigma0_pow_seven, ConwayO7.sigma0_iterate_seven}\leanok
\uses{def:sigma0}
\end{lemma}
...
\lean{ConwayO7.sigma0_fixed_iff}\leanok
\uses{def:sigma0}
\end{lemma}
...
\lean{ConwayO7.sigma0_fixed_card, ConwayO7.sigma0_ne_one}\leanok
...
\lean{ConwayO7.sigma0_cycleType}\leanok
\uses{def:sigma0}
\end{lemma}
...
\lean{ConwayO7.isSRGWith_of_iso, ConwayO7.IsSRGWith.of_iso}\leanok
\uses{def:srg}
\end{lemma}
...
\lean{ConwayO7.exists_sigma0_invariant}\leanok
...
$\Gamma$.
\lean{ConwayO7.seven_not_dvd_card_aut_of_no_sigma0_invariant}\leanok
\uses{def:srg, def:sigma0}
\end{theorem}
